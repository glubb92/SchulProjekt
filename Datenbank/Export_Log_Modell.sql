...
CREATE TABLE IF NOT EXISTS `mydb`.`tblKomponent` (
  ...
  `Gewährleistungsdauer` INT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `mydb`.`tblZulaessiger_Wert` (
  ...
  `Von` DECIMAL NULL COMMENT '',
  `Bis` DECIMAL NULL COMMENT '',
  `Schrittweite` DECIMAL NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `mydb`.`tblZuordnung_Komp_Vorgang` (
  ...
  `Teilkomponenten_ID` VARCHAR(45) NOT NULL COMMENT '',
  PRIMARY KEY (`Komponent_ID`, `Teilkomponenten_ID`)  COMMENT '',
  ...
  INDEX `fk_tblZuordnung_Komp_Vorgang_tblKomponent1_idx` (`Teilkomponenten_ID` ASC)  COMMENT '',
  ...
  CONSTRAINT `fk_tblKomponent_has_tblVorgansart_tblVorgansart1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tblZuordnung_Komp_Vorgang_tblKomponent1`
    FOREIGN KEY (`Teilkomponenten_ID`)
    REFERENCES `mydb`.`tblKomponent` (`Komponent_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
